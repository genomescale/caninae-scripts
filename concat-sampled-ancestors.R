...
sa.tree = read.nhx("concat-total-evidence.mcc.tree")
...
draw.label.above = c(70, 77, 122, 140, 142)

mcc_plot_revts = revts(ggtree(sa.tree))

CairoPDF(file = "concat_sampled_ancestors.pdf", width = 15, height = 15)
mcc_plot_revts %>% flip(140, 141) %>% flip(35, 100) +#%>% flip(97, 101) %>% flip(19, 99) +
